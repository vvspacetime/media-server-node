...
class OutgoingStream
{
	 ...
	 */
	attachTo(incomingStream, waitingForIntra = true)
	{
		//Dettach
		this.detach();
		...
		if (audio.length)
		{
			...
			for (let i=0;i<audio.length && i<tracks.length;++i)
				//Attach them
				transponders.push(audio[i].attachTo(tracks[i], waitingForIntra));
		}
		...
		if (video.length)
		{
			...
			for (let i=0;i<video.length && i<tracks.length;++i)
				...
				transponders.push(video[i].attachTo(tracks[i], waitingForIntra));
		}
	...
	createTrack(params)
	{
		...
		if (params.constructor.name === "TrackInfo")
		{
			...
			if (media=="video")
			{
				...
				source.fec.ssrc	  = params.ssrcs ? params.ssrcs.rtx   : this.lfsr.seq(31);
			} else {
				source.rtx.ssrc	  = 0;
				source.fec.ssrc	  = 0;
			}
		...
		//Create new track
		const outgoingStreamTrack = new OutgoingStreamTrack(media, trackId, Native.TransportToSender(this.transport), source);
